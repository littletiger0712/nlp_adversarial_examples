...
class EntailmentAttack(object):
    ...
    def attack(self, x_orig, target):
        ...
        tmp = [glove_utils.pick_most_similar_words(x2_adv[i], self.dist_mat, 50) if x2_adv[i] != 0 else ([], [])
        ...
        tmp = [glove_utils.pick_most_similar_words(x2_adv[i], self.dist_mat, self.n1) if x2_adv[i] != 0 else ([], [])
               for i in range(len(x2_adv))]
        neighbours_list = [x[0] for x in tmp]
        neighbours_dist = [x[1] for x in tmp]
...
class GeneticAtack(object):
    def __init__(self, model, batch_model,
                 neighbour_model,
                 dataset, dist_mat_order, dist_mat_list,
                 skip_list,
                 lm,
                 pop_size=20, max_iters=100,
                 n1=20, n2=5,
                 use_lm=True, use_suffix=False):
        self.dist_mat_order = dist_mat_order
        self.dist_mat_list = dist_mat_list
    ...
    def select_best_replacement(self, pos, x_cur, x_orig, target, replace_list):
        ...
        # print(np.array(new_x_list))
        new_x_list_str = []
        for new_x in new_x_list:
            new_x_str = ' '.join([str(a) for a in new_x])
            new_x_list_str.append(new_x_str)
        # print(self.neighbour_model.predict(new_x_list_str))
        new_x_scores = np.array(self.neighbour_model.predict(new_x_list_str)[1]).flatten()
        # print('new_x_scores:',new_x_scores)
        x_cur_str = ' '.join([str(a) for a in x_cur])
        orig_score = self.model.predict(x_cur_str)[1]
    ...
    def select_best_replacement_new(self, pos, x_cur, x_orig, target, replace_list):
        """ Select the most effective replacement to word at pos (pos)
        in (x_cur) between the words in replace_list """
        new_x_list = [self.do_replace(
            x_cur, pos, w) if x_orig[pos] != w and w != 0 else x_cur for w in replace_list]
        ...
        new_x_scores = []
        # print('target~~',target)
        for new_x in new_x_list_str:
            score_all = self.neighbour_model.predict(new_x,k=20)
            # print([item.split('_')[-1] for item in list(score_all[0])])
            new_x_index = [item.split('_')[-1] for item in list(score_all[0])].index(str(target))
            new_x_scores.append(score_all[1][new_x_index])
        # print(new_x_scores)
        

        x_cur_str = ' '.join([str(a) for a in x_cur])
        orig_score = self.model.predict(x_cur_str)[1]
        new_x_scores = np.array([-item + orig_score for item in new_x_scores]).flatten()
        # print(new_x_scores)
        if (np.max(new_x_scores) > 0):
            return new_x_list[np.argsort(new_x_scores)[-1]]
        return x_cur
    ...
    def perturb(self, x_cur, x_orig, neigbhours, neighbours_dist,  w_select_probs, target):
        ...
        return self.select_best_replacement_new(rand_idx, x_cur, x_orig, target, replace_list)
    ...
    def attack(self, x_orig, target, max_change=0.4):
        ...
        tmp = [glove_utils.pick_most_similar_words(
            x_orig[i], self.dist_mat_order, self.dist_mat_list, 50, 3.0) for i in range(x_len)]
        ...
        tmp = [glove_utils.pick_most_similar_words(
            x_orig[i], self.dist_mat_order, self.dist_mat_list, self.top_n, 3.0) for i in range(x_len)]
        neigbhours_list = [x[0] for x in tmp]
        neighbours_dist = [x[1] for x in tmp]
        ...
        for i in range(self.max_iters):
            ...
            pop_str_list = []
            for pt in pop:
                pop_str = ' '.join([str(a) for a in pt])
                pop_str_list.append(pop_str)


            pred_labels, probs = self.batch_model.predict(pop_str_list)
            pred_labels = [int(item[0].split('_')[-1]) for item in pred_labels]
            pop_scores = np.array(probs).flatten()
            pop_ranks = np.argsort(pop_scores)
            top_attack = pop_ranks[0]

            # logits = np.exp(pop_scores / self.temp)
            select_probs = (1 / pop_scores) / np.sum(1 / pop_scores)

            if pred_labels[top_attack] != target:
...
class PerturbSentBaseline(object):
    def __init__(self, model, batch_model, dataset, dist_mat,
...
class GreedyAttack(object):
    def __init__(self, model, dataset, dist_mat, lm):
        self.dist_mat = dist_mat
        # self.sess = sess
        self.dataset = dataset
        self.model = model
    ...
    def attack(self,  x_orig, target, max_change=0.4):
        ...
        while ((num_updates / doc_len) < max_change):
            ...
            x_new_pred_probs = np.array(
                [self.model.predict(x[np.newaxis, :])[0] for x in list_x_new])
